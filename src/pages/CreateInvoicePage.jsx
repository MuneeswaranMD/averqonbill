...
export default function CreateInvoicePage() {
    ...
    useEffect(() => {
        ...
        // Compute due date from paymentTerms
        if (settings.paymentTerms && settings.paymentTerms !== 'Immediate' && settings.paymentTerms !== 'Custom') {
            const days = parseInt(settings.paymentTerms.replace('Net ', '')) || 0;
            if (days > 0) {
                const d = new Date();
        ...
        const loadData = async () => {
            ...
            try {
                ...
                try {
                    ...
                    console.warn('Backend products failed to load for invoice', err);
                }
                ...
                console.error('Error loading invoice creation data:', err);
            } finally {
    ...
    const handleSave = async (generatePDF = false) => {
        ...
        try {
            if (customerMode === 'new' && newCustomer.name) {
                await FirestoreService.add('customers', newCustomer, companyId);
            }

            const invoiceNumber = buildDocNumber(
            ...
            const ref = await FirestoreService.add('invoices', invoiceData, companyId);
            const created = { id: ref.id, ...invoiceData };
            setSaved(created);

            // ── Automated Stock Reduction ──
            await FirestoreService.reduceStock(validRows, companyId, `Invoice ${invoiceNumber}`);

            // 3. Trigger Automation
            AutomationService.trigger(companyId, 'invoice.created', created);
    ...
    if (saved) {
        ...
        return (
            <div className="max-w-xl mx-auto py-16 flex flex-col items-center text-center px-4">
                ...
                <h1 className="text-xl font-bold text-gray-900 mb-2">Invoice Created Successfully</h1>
                <p className="text-sm text-gray-500 mb-1">Invoice Number: <span className="font-mono font-semibold text-gray-700">{saved.invoiceNumber || `INV-${String(saved.id).slice(-8).toUpperCase()}`}</span></p>
                <p className="text-sm text-gray-500 mb-6">Customer: <strong>{saved.customerName}</strong> &middot; Total: <strong className="text-blue-700">{currSym}{fmt(saved.totalAmount)}</strong></p>
                ...
                <div className="flex flex-wrap gap-3 justify-center">
                    ...
                    >
                        <Plus size={15} /> Create Another Invoice
                    </button>
            ...
            <div className="flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-sm text-gray-500">Generate a new tax invoice for your customer</p>
                    </div>
                </div>
            </div>
